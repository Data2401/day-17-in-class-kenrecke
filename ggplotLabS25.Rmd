...
ggplot(heights, aes(x = height, fill = sex)) + geom_boxplot(alpha = 1/2)
```
...
heights %>% summarise(mean(height),median(height))
...
```{R}
heights %>% group_by(sex) %>% summarise(max(height)) 
...
```{r}
v <- votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote))
...
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote)) + coord_flip()
...
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x = reorder(state, demVote))) + coord_flip()
...
```{r}
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote))) + coord_flip() + facet_wrap(~ south)
...
```{r}
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote), color = state)) + coord_flip()
...
```{r, eval = F}
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote, color = state))) + coord_flip() + labs(title = "Voting Date", subtitle = "Democratic Votes per State", xlab = "Votes", ylab = "State")
...
```{r, eval = F}
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote, color = state))) + coord_flip() + labs(title = "Voting Date", subtitle = "Democratic Votes per State", xlab = "Votes", ylab = "State") + theme(legend.position = "bottom")
...
votedata %>% ggplot(aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote, color = state))) + coord_flip() + labs(title = "Voting Date", subtitle = "Democratic Votes per State", xlab = "Votes", ylab = "State") + facet_wrap(~ year)
...
```{r}
ggplot(votedata, aes(x = state)) + geom_col(aes(y = demVote, x= reorder(state, demVote, color = state), geom_vline(50))) + coord_flip() + labs(title = "Voting Date", subtitle = "Democratic Votes per State", xlab = "Votes", ylab = "State") + facet_wrap(~ year) 
...
```{r}
map_plot <-  ggplot(data =  votedata %>% filter(south == TRUE), aes(map_id = state)) +
  geom_map(aes(fill = demVote), map = states_map)+
  expand_limits(x = states_map$long, y = states_map$lat)
map_plot 
